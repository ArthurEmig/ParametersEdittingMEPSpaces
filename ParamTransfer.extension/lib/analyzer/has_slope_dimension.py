...
NAME = 'Check if a sloped pipe has slope spot tag'
# Short description of the collector
DESCRIPTION = 'Check if a sloped pipe has slope spot tag'
# Author
AUTHOR = 'Arthur Emig'
...
"""
class HasSlopeDimensionAnalyzer(Analyzer):
    name = NAME
    _author = AUTHOR
    ...
    def analyze(self, data, **kwargs):
        ...
        for element in data:
            ...
            item = ReportItem(
                ...
                passed=has_slope_dimension(elem=element, view_id=activeview_id),
            )
...
def has_slope_dimension(elem, view_id):
    """
    Check if the elememt (elem) has hosted independent tags.
    """
    filter_ = db.ElementClassFilter(db.SpotDimension)

    slope_dimensions = elem.GetDependentElements(filter_)

    slope_dimensions_viewids = [DB.Document.GetElement(doc,eid).OwnerViewId for eid in slope_dimensions]

    if view_id not in slope_dimensions_viewids:
        return False

    return bool(slope_dimensions)
...
"""
export = HasSlopeDimensionAnalyzer()
